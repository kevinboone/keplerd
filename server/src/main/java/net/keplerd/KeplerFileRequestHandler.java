...
*/
public class KeplerFileRequestHandler extends HandlerImpl
  {
  ...
  public void handle (Request request, Response response) throws IOException
    ...
    if (config.isDebug())
      Logger.tag ("error").debug ("Full path = " + fullPath);

    Principal p = request.getUserPrincipal();
    String ident = getIdentFromPrincipal (p);
    ...
    if (fullPath.exists())
      ...
      if (fullPath.isDirectory())
        ...
        if (!config.getSecurityEnabled() || ka.isAllowed (ident))
          ...
          if (unchanged)
            {
            if (config.isDebug())
              Logger.tag ("error").debug ("Sending an \"unchanged\" response");
	  ...
	  response.setErrorMessage ("Not authorized: " + path);
        ...
        Logger.tag ("error").warn ("Request for non-file, non-dir: " + path);
        }
      }
    else
      {
